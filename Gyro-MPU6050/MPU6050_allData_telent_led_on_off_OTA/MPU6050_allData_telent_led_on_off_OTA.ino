...
String page = "";
double data;
unsigned long previousMicros = 0;        // will store last temp was read
...
float data_rate = 100000;           // data transmission rate in microseconds
...
//uint32_t count = 0;
uint32_t delt_t = 0;
//float gyroBias[3] = {0.08, -0.36, 0.30}, accelBias[3] = { -0.05, -0.04, 0.31};
float gyroBias[3] = { -3.73, -0.91, -1.86}, accelBias[3] = { -0.08, 0, 0.05};
...
void setup()
{
  ...
  webserver.on("/", SendIP);
  webserver.on("/data", SendData);
  webserver.on("/start", StartMeasurement);
  //  webserver.on("/stop", StopMeasurement);
...
//    MadgwickQuaternionUpdate(ax, ay, az, gx * PI / 180.0f, gy * PI / 180.0f, gz * PI / 180.0f);
  ...
  if (startflag) {
    ...
    if (flag && (currentMicros - previousMicros > data_rate)) {
      ...
      //    Serial.println(q[3]);//Serial.println('\t');
      ...
      String str = String(gx) + "\t" + String(gy) + "\t" + String(gz) + "\t" + String(ax) + "\t" + String(ay) + "\t" + String(az) + "\t" + String(analogRead(A0)) + "\t" + String(elapsedMillis) + "\t" + String(digitalRead(ledPin)) + "\t" + String(q[0]) + "\t" + String(q[1]) + "\t" + String(q[2]) + "\t" + String(q[3]);// + "\n";
...
void StartMeasurement() {
  if (!startflag) {
    ...
    //        Serial.println(c, HEX);
    //        //      while (1) ; // Loop forever if communication doesn't happen
    //        page = "Could not connect to MPU6050: 0x" + String(c);
